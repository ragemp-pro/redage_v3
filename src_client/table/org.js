const
    clientName = "client.org.main.",
    rpcName = "rpc.org.main.",
    serverName = "server.org.main.";
...
gm.events.add(clientName + "vehicles", (isVehicleUpdateRank, _vehicles, isSellVehicle) => {

    _vehicles = table.getVehicles(_vehicles);

    mp.gui.emmit(`window.listernEvent ('table.vehicles', ${isVehicleUpdateRank}, \`${JSON.stringify(_vehicles)}\`, ${isSellVehicle});`);
});

gm.events.add(clientName + "sellCar", (number) => {
    if (!global.antiFlood("table.sellCar", 500))
        return;

    mp.events.callRemote(serverName + "sellCar", number);
});
...
gm.events.add(clientName + "disolver", () => {
    global.binderFunctions.GameMenuClose ();
    mp.events.callRemote(serverName + "disolver");
});


gm.events.add(clientName + "saveSetting", (slogan, value, discord, colorR, colorG, colorB) => {
    if (!global.antiFlood("table.saveSetting", 1000))
        return;

    mp.events.callRemote(serverName + "saveSetting", slogan, value, discord, colorR, colorG, colorB);
});

gm.events.add(clientName + "setLeader", (uuid) => {
    global.binderFunctions.GameMenuClose ();
