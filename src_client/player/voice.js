	...
	constructor () {
		...
		this.sendAddData = (gm.createTimeout)(() => {
			if (this.queueAddList.size) {
				mp.events.callRemote("add_voice_listeners", JSON.stringify(Array.from(this.queueAddList)))
		...
		this.sendRemoveData = (gm.createTimeout)(() => {
			if (this.queueRemoveList.size) {
				mp.events.callRemote("remove_voice_listeners", JSON.stringify(Array.from(this.queueRemoveList)));
