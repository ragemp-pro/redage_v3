...
damageController.addBone(0,			    1); // 0 - таз
damageController.addBone(11816,          1); // 1 - нога
damageController.addBone(58271,          1); // 2 - голень
damageController.addBone(63931,          1); // 3 - ступня
damageController.addBone(14201,          1); // 4 - нога
damageController.addBone(2108,           1); // 5 - голень
damageController.addBone(65245,          1); // 6 - ступня
damageController.addBone(57717,	        1); // 7 - живот-таз
damageController.addBone(64157,		    1); // 8 - живот
damageController.addBone(60734,			1); // 9 - грудь
damageController.addBone(4115, 		    1); // 10 - грудь-горло
damageController.addBone(46078,		    1); // 11 - правое плечо
damageController.addBone(50201,		    1); // 12 - правое предплечье
damageController.addBone(24589,		    1); // 13 - правая рука
damageController.addBone(51826,		    1); // 14 - правая ладонь
damageController.addBone(36864,			1); // 15 - левое плечо
damageController.addBone(52301,			1); // 16 - левое предплечье
damageController.addBone(20781,			1); // 17 - левая рука
damageController.addBone(35502,			1); // 18 - левая ладонь
damageController.addBone(24806,			1); // 19 - горло
damageController.addBone(34414,			1); // 20 - голова
...
mp._events.add('outgoingDamage', (sourceEntity, targetEntity, sourcePlayer, weapon, boneIndex, damage) => {
    ...
    if (global.weaponCategory > 0 && damageData.category != global.weaponCategory)
        return true;
...
mp._events.add('incomingDamage', (sourceEntity, sourcePlayer, targetEntity, weapon, boneIndex, damage) => {
    ...
    if (targetEntity === mp.players.local) {
        ...
        if (!damageData.isMelee) {
            ...
            mp.players.local.applyDamageTo(customDamage + 1, true);
            return true;
        }

        mp.game.weapon.setCurrentDamageEventAmount(customDamage);
    }
