...
const isNotAttack = () => {
	if ([7, 9].includes (global.fractionId) && [911657153].includes (global.weaponData.weapon)) 
		return false;
	else if ([600439132, 126349499, WEAPON_STUNGAN].includes (global.weaponData.weapon))
		return false;
...
const getNearestLookingObject = () => {
	try {
		...
		mp.objects.forEachInStreamRangeItems(object => {
			...
			if (objectDist <= 2.2 && playerDist <= 3.0) {
				objects.push({
					'distance': objectDist,
					'entity': object
				});
...
const GetLooking = (isObj = false, isBtn = false) => {
	try {
		...
		/*if (!isBtn && global.entity !== null && global.entity.doesExist()) {
			...
			//if (mp.game.gameplay.getDistanceBetweenCoords(ePosition.x, ePosition.y, ePosition.z, pPosition.x, pPosition.y, pPosition.z, true) <= 3.0) return global.entity;
		}
		*/
		...
		if (!global.localplayer.vehicle) {
			...
			if ((getEntity || ((isObj || isBtn) && (getEntity = getNearestLookingObject()))) && getEntity && getEntity.type && [
				...
				if (getEntity === global.entity) 
					return getEntity;
				if ("player" === getEntity.type && !global.localplayer.hasClearLosTo(getEntity.handle, 17)) {
					global.entity = null;
					return null;
				}
				...
				if ("player" === getEntity.type && (getEntity['INVISIBLE'] || global.GetGender (getEntity) === -1)) {
					global.entity = null;
					return null;
				}
...
//
/*
gm.interaction = new class {
	constructor() {
		this.interactionDistance = 2.5;

		gm.events.add("ready", () => {
			//mp.game.graphics.hasStreamedTextureDictLoaded("commonmenu") || mp.game.graphics.requestStreamedTextureDict("commonmenu", !0);
			this.load();
		});

		gm.events.add("render", () => {
			if (global.cameraManager.gameplayCam()) {
				if (this.entityWorldPosition) {
					const position = mp.game.graphics.world3dToScreen2d(this.entityWorldPosition);
					if (position) {
						const ratio = mp.game.graphics.getScreenAspectRatio(true);
						mp.game.graphics.drawSprite("commonmenu", "common_medal", position.x, position.y, 0.01, 0.01 * ratio, 0, 256, 256, 256, 128);
					}
				} else if (mp.players.local.vehicle && mp.game.controls.isControlJustPressed(0, 86))
					this.processHornAction();
			}
		});

	}
	load() {
		gm.timers.setInterval(() => {
			delete this.entityWorldPosition;
			delete this.entity;
			if (!global.cameraManager.gameplayCam()) return;
			if (!this.isAvailable()) return;
			const e = this.getRaycastData(),
				r = this.getEntityFromRaycastData(e);
			r && this.isInteractionEntity(r) && this.isEntityInInteractionDistance(r) && ((this.entity = r), this.updateEntityWorldPosition());
		}, 200);
	}
	getRaycastData() {
		const getDirection = gm.player.camera.gameplayCamera.getDirection();
		const getBoneCoords = mp.players.local.vehicle ?
			mp.players.local.getBoneCoords(31086, 0.2, 0, 0) :
			mp.players.local.getBoneCoords(11816, -0.2, 0, 0);

		const a = getDirection.multiply(this.interactionDistance).add(getBoneCoords);
		return mp.raycasting.testPointToPoint(getBoneCoords, a, mp.players.local, RAYCASTING_FLAGS.vehicles | RAYCASTING_FLAGS.players);
	}
	getEntityFromRaycastData(e) {
		if (!e) return null;
		let r = null;
		if ("number" == typeof e.entity) {
			const a = gm.player.peds.get({ handle: e.entity });
			a && a.props && a.props.interaction && (r = this.getPedEntity(a.props));
		} else t.y.IS_ENTITY_VISIBLE(e.entity.handle) && (r = e.entity);
		return r;
	}
	isInteractionEntity(e) {
		return ["vehicle", "player", "ped"].includes(e.type);
	}
	updateEntityWorldPosition() {
		const e = new mp.Vector3(0, 0, 1);
		if ("vehicle" === this.entity.type) {
			const r = mp.game.gameplay.getModelDimensions(this.entity.model);
			e.z = r.max.z;
		}
		this.entityWorldPosition = utils.makeVector3(this.entity.position).add(e);
	}
	isAvailable() {
		return (
			!mp.players.local.isFaint &&
			!gm.player.actions.isPlayerInvisible(mp.players.local) &&
			!!gm.player.actions.isVisible(mp.players.local) &&
			!gm.vehicleShove.isShovedInTrunk() &&
			!(!gm.competitions || gm.competitions.isOnCompetition()) &&
			!gm.inventories.isInteractionDisabled()
		);
	}
	isEntityInInteractionDistance(e) {
		const r = 2 * this.interactionDistance,
			a = 3 * this.interactionDistance;
		let t = r;
		if (mp.vehicles.exists(e)) {
			const o = mp.game.gameplay.getModelDimensions(e.model),
				i = Math.abs(o.min.y) + o.max.y;
			(t = r), i > t && (t = i), t > a && (t = a);
		}
		return utils.isPositionsInRadius(e.position, mp.players.local.position, t);
	}
	processHornAction() {
		gm.player.peds
			.filter((e) => !1 !== gm.player.peds.getProps({ index: e.index }).hornInteraction)
			.some((e) => {
				const r = gm.player.peds.getProps({ index: e.index });
				return !!utils.isPositionsInRadius(mp.players.local.position, r.position, r.hornInteraction) && ((this.entity = this.getPedEntity(r)), !0);
			});
	}
	getPedEntity(e) {
		return { ...e, type: "ped", isPed: !0 };
	}
}
